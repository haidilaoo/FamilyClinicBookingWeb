...
function validateSignupForm(isSignup) {
    ...
    if (!isSignup) {
        ...
        if (password) {
            ...
            if (!checkLastName(lastname)) valid = false;
            if (!checkEmail(email)) valid = false;
            if (!matchPassword(password)) valid = false; //so that form input information is sent to php once button is clicked and not stopped by validation js
...
function matchPassword(password) {
    ...
    if (password != currentPassword) {
        document.getElementById("passwordError").innerText = "Incorrect current password." + currentPassword ;
        console.log("hash input: " + hashed_inputPassword);
        return false;
    }
    // else if (password.length == currentPassword) {
        // document.getElementById("passwordError").innerText = "";
        // return false;
    // } 
    else {
