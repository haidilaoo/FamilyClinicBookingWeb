...
<?php  include "../php snippets/navbar.php"; ?>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Now you can use $appointment_id in your PHP code
    ...
    if ($result && $result->num_rows > 0) {
        // Fetch the doctor_id from the result set
        $row = $result->fetch_assoc(); // Fetch as associative array
        $doctor_id = $row['doctor_id']; // Get the doctor_id from the row
        echo "doctorid: " . $doctor_id; // Output the doctor_id

        //set session variable
        $_SESSION['selected_doctor'] = $doctor_id;
        
    } else {
        echo "No doctor found for the given appointment ID.";
    }
    ...
    <section style="margin-top: 100px;">
        <div class="two-section">
            <div class="left-col">
                ...
                <h2>Reschedule a date & time slot</h2>
                ...
                <div class="card display-flex gap-24">
                    ...
                    $sql = '
                        SELECT d.first_name, d.last_name, d.specialty, d.doctor_image 
                        FROM Doctors d
            ...
            <div class="right-col">
                <div class="right-col-width">
                    <form method="POST" id="date-time-form" action="../php snippets/updateappt.php">
                        ...
                        foreach ($datesByMonth as $monthYear => $dates) {
                            ...
                            foreach ($dates as $date) {
                                ...
                                echo "<div class='date-group' data-month='$monthYear'>"; // Each month has a data-month attribute


                                echo "<input type='radio' name='weekly-date-selector' id='d$date' value='$date' $disabledAttribute onClick=\"showAppSlot('$date')\"/>";
                        ...
                        <div class="display-flex align-items-right">
                            <input type="submit" class="btn-blue-sm btn-margintop" value="Reschedule">
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    function showAppSlot(date) {
        ...
        for (var i = 0; i < timeslotsData[date].length; i++) {
            var newRadioHTML = '<input type="radio" name="timing-selector" id="t' + timeslotsData[date][i] + '" value="' + timeslotsData[date][i] + '">';
